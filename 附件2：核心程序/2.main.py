...
def parse_args():
    ...
    parser.add_argument('--inputgraph', nargs='?',
                        default='D:\\lxw\\AttriRank-master\\sample\\graph.edgelist',
                        help='Input graph path')

    parser.add_argument('--inputfeature', nargs='?',
                       default='D:\\lxw\\AttriRank-master\\sample\\graph.feature',
                        help='Input feature path')

    parser.add_argument('--output', nargs='?', default='graph.rankscore0.85',
                        help='Output rankscore path')#输出到当前路径

    parser.add_argument('--kernel', default='rbf_ap',
                        help='Kernel: rbf_ap, rbf, cos, euc, sigmoid')

    parser.add_argument('--damp', nargs='*', default=[0.85], type=float,
                        help='damping parameters')#阻尼系数
